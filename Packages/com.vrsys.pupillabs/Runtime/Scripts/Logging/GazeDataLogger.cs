...
namespace VRSYS.PupilLabs
{
    ...
    public class GazeDataLogger : MonoBehaviour
    {
        private const string GazeCsvHeader =
            ...
            "calibrated_gaze_dir_x,calibrated_gaze_dir_y,calibrated_gaze_dir_z," +
            "calibrated_gaze_origin_x,calibrated_gaze_origin_y,calibrated_gaze_origin_z," +
        ...
        #region Eye-Tracker Connection Methods
        ...
        private void OnGazeDataReceived(object sender, EventArgs e)
        {
            ...
            string calibratedGazeDirX = string.Empty, calibratedGazeDirY = string.Empty, calibratedGazeDirZ = string.Empty;
            string gazeOriginX = string.Empty, gazeOriginY = string.Empty, gazeOriginZ = string.Empty;
            if (_dataStorage != null && _dataStorage.CameraIntrinsics != null)
            {
                ...
                Vector3 calibratedGazeDir = _subscribedProvider.GazeOrigin.rotation * rawGazeDir;
                calibratedGazeDirX = calibratedGazeDir.x.ToString("F6", CultureInfo.InvariantCulture);
                calibratedGazeDirY = calibratedGazeDir.y.ToString("F6", CultureInfo.InvariantCulture);
                calibratedGazeDirZ = calibratedGazeDir.z.ToString("F6", CultureInfo.InvariantCulture);

                gazeOriginX = _subscribedProvider.GazeOrigin.position.x.ToString("F6", CultureInfo.InvariantCulture);
                gazeOriginY = _subscribedProvider.GazeOrigin.position.y.ToString("F6", CultureInfo.InvariantCulture);
                gazeOriginZ = _subscribedProvider.GazeOrigin.position.z.ToString("F6", CultureInfo.InvariantCulture);
            }
            ...
            lock (_writerLock)
            {
                ...
                _writer.WriteLine(string.Join(",",
                    ...
                    calibratedGazeDirX, calibratedGazeDirY, calibratedGazeDirZ,
